...
import { approveChapter, rejectChapter } from "./actions";

function SubmitButton({ label, variant = "default" }: { label: string; variant?: "default" | "destructive" | "outline" }) {
...
export function ChapterActions({ id }: { id: string }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <form action={approveChapter}>
        <input type="hidden" name="id" value={id} />
        <SubmitButton label="Approve" />
      </form>

      {rejecting ? (
        <form action={rejectChapter} className="flex flex-wrap items-center gap-2">
          <input type="hidden" name="id" value={id} />
          <input
            type="text"
            name="reason"
