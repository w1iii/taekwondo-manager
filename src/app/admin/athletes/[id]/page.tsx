...
export default async function AthleteDetailAdminPage({
  params,
}: {
  ...
  const payments = await db.teamPayment.findMany({
    where: { chapterId: athlete.chapterId, eventId: { in: eventIds } },
  });
    ...
    <div className="mx-auto w-full max-w-2xl space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{athlete.chapter.name}</p>
        </div>
        <Button render={<Link href="/admin/athletes" />} variant="outline">
          <ArrowLeft />
          Back to athletes
        </Button>
      </div>
